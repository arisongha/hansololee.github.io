...
설명을 편하게 하기위해서 $$v_t$$의 순서를 역순으로 했습니다. 맨 위에 $$v_{100}$$ 에 대한 식은 $$v_{99}$$를 대체해주므로써 아래와 같이 표현할 수 있습니다.

$$
v_{100} = 0.1\theta_{100} + 0.9(0.1\theta_{99} + 0.9v_{98})
$$

그리고 위 식의 $$v_{98}$$ 을 또 다음과 같이 대체할 수 있습니다.
...
마지막 식을 통해서 규칙성을 파악하셨겠지만 $$v_t$$ 는 이전 데이터에 지수적으로 감소하는 함수를 곱해주고 모두 더해주는 것입니다. $$\theta$$ 앞의 계수들을 모두 더하면 1 또는 1에 가까운 값이 됩니다. 이는 아래서 자세히 설명드릴 **편향보정** 이라고 불리는 값 입니다. 이 값들에 의해 지수가중평균이 되는 것입니다. 그럼 $$v_t$$ 가 얼마나 많은 데이터의 평균값이 되는지 궁금하실 겁니다. 위 예시에서 사용한 $$\beta$$ 값인 0.9의 10거듭제곱 값은 대략적으로 0.35와 같습니다. 이는 $$\frac{1}{e}$$ 값과도 대략적으로 같습니다. 일반적으로 표현하자면

$$1 - \epsilon = \beta$$
$${(1 - \epsilon)}^{\frac{1}{\epsilon}} = \frac{1}{e}$$

 즉, $$\frac{1}{e}$$ 값이 되기까지 이전 10개의 데이터가 필요한 것입니다. 이를 한 눈에 이해하기 위해서 아래 그림을 확인해 보겠습니다.

<br/>
<center><img data-action="zoom" src='{{ "/assets/img/exp_weighted_average_04.jpg" | relative_url }}' alt='absolute'></center>
<br/>

위 그림과 같이 10개 이전의 데이터의 가중치가 현재 가중치의 약 1/3로 줄어드는 것입니다. 이전에 예시로 들었던 $$\beta$$ 값인 0.98의 경우 $${0.98}^{50}$$ 이 대략적으로 $$\frac{1}{e}$$ 와 같습니다. 이는 이전에 언급했던 $$\frac{1}{1-\beta}$$ 를 관습적으로 쓰는 이유입니다. $$1/(1-0.98)=50$$ 이기 때문이지요.

실제로 구현할 때는 $$v_0, v_1, v_2, ...$$ 을 모두 변수화 하지 않습니다. 아래와 같이 $$v_{\theta}$$ 값을 덮어씌워 줍니다. ($$v_{\theta}$$ 로 표시한 이유는 $$\theta$$ 를 매개변수로 하는 지수가중이동평균을 계산한다는 것을 나타내기 위함입니다.)
...
이렇게 $$v_{\theta}$$는 매번 업데이트 됩니다. 이렇게 지수평균값을 얻는 장점은 아주 적은 메모리를 사용한다는 점입니다. 가장 최근에 얻은 값을 계속 덮어쓰기만 하면 되니까요. 이 방법은 컴퓨터 계산비용과 메모리 효율 측면에서 더 효율적이기 때문에 오늘날 머신러닝에서 많이 사용하고 있습니다.
